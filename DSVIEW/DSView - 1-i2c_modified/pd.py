...
class Decoder(srd.Decoder):
    ...
    options = (
        ...
        {'id': 'data_only', 'desc': 'Read data & ack only',
            'default': 'False', 'values': ('False', 'True'), 'idn':'...'},
        {'id': 'cpol', 'desc': 'CLK polarity(reverse or not)',
            'default': 0, 'values': (0, 1), 'idn':'...'},
        {'id': 'dpol', 'desc': 'Data polarity(reverse or not)',
            'default': 0, 'values': (0, 1), 'idn':'...'},
    )
    annotations = (
    ...
    annotation_rows = (
        ('bits', 'Bits', (5,)),
        ('addr-data', 'Address/Data', (0, 1, 2, 6, 7, 8, 9)),
        ('ack/nack', 'ACK/NACK', (3, 4)),
    ...
    def start(self):
        ...
        self.dpol = self.options['dpol']
        self.cpol = self.options['cpol']
    ...
    def decode(self):
        mode = self.options['data_only']

        if mode == 'True': 
            self.state = 'FIND DATA'
            scl_edge = 'r' if self.cpol==0 else 'f'
            scl_edge_st = 'f' if self.cpol==0 else 'r'
            self.wait({0: scl_edge_st})  # to edge start

            while True:
                if self.state == 'FIND DATA':              
                    (scl, data) = self.wait({0: scl_edge})
                    start = self.samplenum  
                    self.wait({0: scl_edge_st})
                    end = self.samplenum

                    # print out stop at same column for simulation later
                    if (end - start) > (self.samplerate * 24e-6):   
                        self.ss = start
                        self.es = end
                        self.putx([8,  ['Stop']]) 
                        self.putx([proto['ACK'][0], ['None']])  
